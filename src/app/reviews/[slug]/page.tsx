...
export default async function Page({
  ...
  const {alt, disclaimer, review}: Config = await getDocument<Config>("config", "config");
  let altObj: {[key: string]: any} = global.documentCache?.get(`review/${slug}/altObj`) ?? undefined;
  if(altObj === undefined){
    altObj = review.dispalyed_column_ratings.reduce((obj: { [key: string]: any; }, curr: string) => ({ ...obj, [curr]: getAltString(alt["review"], curr, company[curr]) }), {});
    global.documentCache?.set(`review/${slug}/altObj`, altObj,604800000);
  }
  ...
  return (
    <Article className="container mx-auto py-10 review-summary" announcement={""}>
      <>
        {date && (
        ...
        <h1 className="text-3xl md:text-2xl text-center font-bold my-1">{company.name}</h1>
        <div
            id="review-data-list"
