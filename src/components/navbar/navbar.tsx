  ...
  if (isNavOpen) {
    return (
      <FocusTrap id="navbar-menu" as="div" className={"flex flex-col w-full h-screen bg-white"}>
        <button
          className="self-end cursor-pointer pr-4 text-gray-500 md:hidden"
                  ...
                  <ol className="text-center">
                    {link.children?.map((child: LinkType, j: number) => (
                      <li key={j} className="my-4 px-2">
                        <NavItem href={child.url} name={child.name} className="!inline-block text-black text-xl text-center rounded" target={child.target} onClick={() => setNavOpen(!isNavOpen)}/>
                      </li>
                    ))}
                  </ol>
                </Accordion>
