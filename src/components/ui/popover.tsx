...
export default function Popover(
    { children, toggle, className, props}: Readonly<{
    ...
    return (
        <div className={`relative flex flex-row justify-center ${className?.popover ?? ''}`}>
            <button
                ref={triggerRef}
                onClick={popoverVisibilityHandler}
                className={`cursor-pointer rounded pt-2 ${className?.toggle ?? ''}`}
                aria-haspopup="true"
